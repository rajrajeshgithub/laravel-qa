...
jQuery(document).ready(function()
{
	...
	jQuery('#loadMore').on('click',function() {
		...
		$.ajax(
		{
			type:'POST',	
			cache:false,
			url:SITEURL+'ut2myaccount/getFundraiserCommentByAjax',
			data:dataArr,
	...
	jQuery(document.body).on('click', '.editComment' ,function(){		
		...
		jQuery(this).parents('.comment-box').find('.comment-content').text(" ").append("<div class='edit-content-box'><textarea name='commentContent' id='commentContent' class='edit-input'></textarea></div>")
	...
	jQuery(document.body).on('click', '.saveComment' ,function(){	
		...
		jQuery.ajax( {
			type:'POST',
			data:dataArr,
			url:SITEURL+'ut2myaccount/updateFundraiserComment',
			cache:false,
			beforeSend:function() {
	...
	jQuery(document.body).on('click', '.approveComment' ,function(){	
		...
		jQuery.ajax( {
			type:'POST',
			data:dataArr,
			url:SITEURL+'ut2myaccount/approveFundraiserComment',
			cache:false,
			beforeSend:function(){},
			success:function(res) {
	...
	//jQuery('.removeComment').on('click',function() {
		...
		if(confirm('Are to sure to delete this comment...?')) {
			...
			jQuery.ajax( {
				type:'POST',
				data:dataArr,
				url:SITEURL+'ut2myaccount/deleteFundraiserComment',
				cache:false,
				beforeSend:function(){},
				success:function(res) {
...
function getMsgHtml(resStatus,type) {
	...
	msgHTML+='<div id="msgSection" class="margin-top-10">';
	msgHTML+=msgText;
	msgHTML+='</div>';
	return msgHTML;
}

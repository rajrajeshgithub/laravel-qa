<?php
	class UserType2_Model extends Model {
		public $userId, $userDetailsArray, $LoginUserDetail, $widgetId;
		public $Pstatus = 1;
		public $Ein, $NPOID, $ConfirmCode, $FirstName, $LastName, $CompanyName, $Designation, $Address1, $Address2, $City, $State, $Country, $Zip, $PhoneNumber, $Mobile, $EmailAddress, $Password, $ConfirmationPassword;
		...
		public $ExistPassword, $Pageurl, $DateTime, $uid, $ConfirmPassword;
		
		function __construct() {
			$this->LoginUserDetail = getSession('Users', 'UserType2');
			$this->uid = isset($this->LoginUserDetail['user_id']) ? keyDecrypt($this->LoginUserDetail['user_id']) : 0;
		}
		
		public function ValidateAddUser() {
			if(trim($this->FirstName) == '') {
				$this->SetStatus(0, 'E4003');
				$this->Pstatus = 0;
			}
			if(trim($this->LastName) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4004');
				$this->Pstatus = 0;
			}
			if(trim($this->EmailAddress) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4005');
				$this->Pstatus = 0;
			}
			if(trim($this->CompanyName) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4006');
				$this->Pstatus = 0;
			}
			if(trim($this->PhoneNumber) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4008');
				$this->Pstatus = 0;
			}
			if(trim($this->City) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4009');
				$this->Pstatus = 0;
			}
			if(trim($this->State) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4010');
				$this->Pstatus = 0;
			}
			if(trim($this->Country) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4011');
				$this->Pstatus = 0;
			}
			if(trim($this->Address1) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4012');
				$this->Pstatus = 0;
			}
			if(trim($this->Password) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4013');
				$this->Pstatus = 0;
			}
			if((trim($this->Password) != trim($this->ConfirmPassword))  && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4014');
				$this->Pstatus = 0;
			}
		}
		...
		public function AddUser() {
			...
			if($this->Pstatus) {
				$DataArray = array(
					...
					'RU_RegDate'		=>$this->RegDate,
					'RU_UpdatedDate'	=>$this->UpdateDate,
					'RU_UserIP'			=>$this->UserIP,
				...
				if($this->userId > 0) {
					...
					$DataArray = array(
						...
						'NPOEIN' => $this->Ein,
						'USERID' => $this->userId,
						'NPOConfirmationCode' => $this->ConfirmCode,
						'Status' => '0',
						'RegistrationDate' => $this->RegDate,
						'Log' => $Log,
						'CreatedDated' => $this->RegDate,
						'LastUpdatedDate' => $this->UpdateDate,
						'Active' => '1');
		...
		public function GetNPOProfileDetail($DataArray, $UserId) {
			...
			$Sql = "SELECT $Fields FROM " . TBLPREFIX . "npouserrelation NUR LEFT JOIN " . TBLPREFIX . "npodetails N ON (NUR.NPOEIN=N.NPO_EIN)";
		...
		public function Processlogin() {
			if($this->Pstatus)
				$this->ValidateLogin();	
			if($this->Pstatus)
				$this->updateAcessDate();
			if($this->Pstatus)
				$this->SetSession();
		}
		
		public function ValidateLogin() {
			if($this->EmailID == '' && $this->Pstatus == 1) {
				$this->Pstatus = 0;
				$this->SetStatus(0, 'E2001');
			}
			if($this->Password == '' && $this->Pstatus == 1) {
				$this->Pstatus = 0;
				$this->SetStatus(0, 'E2002');
			}
			if($this->VerifyLogin() == false && $this->Pstatus == 1) {
				$this->Pstatus = 0;
				$this->SetStatus(0, 'E2003');
			}	
			if($this->matchPassword() == false && $this->Pstatus == 1) {
				$this->Pstatus = 0;
				$this->SetStatus(0, 'E2004');
			}
		}
		...
		public function SetSession() {
			$DetailArr	= array(
				'user_id' => keyEncrypt($this->userDetailsArray["RU_ID"]),
				'user_firstname' => $this->userDetailsArray["RU_FistName"],
				'user_lastname' => $this->userDetailsArray["RU_LastName"],
				'user_email' => $this->userDetailsArray["RU_EmailID"],
				'user_fullname' => $this->userDetailsArray["RU_FistName"] . " " . $this->userDetailsArray["RU_LastName"],
				'user_ambassador' => $this->userDetailsArray["RU_AllowAmbassador"],
		...
		public function GetNPODetail($DataArray, $Condition=NULL) {
			...
			$Fields	= implode(",", $DataArray);
			$Sql = "SELECT $Fields FROM " . TBLPREFIX . "npodetails N LEFT JOIN " . TBLPREFIX . "npouserrelation NUR ON (N.NPO_EIN=NUR.NPOEIN) LEFT JOIN " . TBLPREFIX . "registeredusers RU ON (RU.RU_ID=NUR.USERID)";
			$Res = db::get_row($Sql . $Where);
			return $Res;		   
		}
		...
		public function checkLogin($loginDetail) {
			...
			if(is_array($loginDetail['UserType2'])) {
				if(isset($loginDetail['UserType2']['is_login']) && $loginDetail['UserType2']['is_login'] == 1) {
		...
		public function ValidateChangePasswordInput() {
			if(trim($this->ExistPassword) == '' && $this->Pstatus == 1) {
				$this->Pstatus = 0;
				$this->SetStatus(0, 'E10004');
			}
			if(trim($this->Password) == '' && $this->Pstatus == 1) {
				$this->Pstatus = 0;
				$this->SetStatus(0, 'E10005');
			}			
			if(trim($this->Password) != trim($this->ConfirmPassword)) {
				$this->Pstatus = 0;
				$this->SetStatus(0, 'E10001');
			}	
			if(!$this->CheckCurrentPasswordExistance($this->ExistPassword) && $this->Pstatus == 1) {
				$this->Pstatus = 0;
				$this->SetStatus(0, 'E10006');
			}
		}
		...
		/* dashboard */
		public function GetUserDetails($DataArray = array('RU.RU_ID','RU.RU_FistName','RU.RU_LastName','RU.RU_EmailID','RU.RU_ProfileImage','NRU.NPOID','RU.RU_City','RU.RU_ZipCode','RU.RU_Address1','RU.RU_Address2','RU.RU_EmailID','RU.RU_AllowAmbassador'), $Condition=NULL) {
			$Fields	= implode(",", $DataArray);
			$Sql = "SELECT $Fields FROM " . TBLPREFIX . "registeredusers RU LEFT JOIN " . TBLPREFIX . "npouserrelation NRU ON (RU.RU_ID=NRU.USERID)";
			$Where	= " WHERE 1";
			$Res = array();
			if($Condition == NULL) {
				$this->LoginUserDetail = getSession('Users', 'UserType2');
		...
		public function ValidateManageProfileInput() {
			if(trim($this->FirstName) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E11001');
				$this->Pstatus = 0;
			}
			if(trim($this->LastName) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E11002');
				$this->Pstatus = 0;
			}				
			if(trim($this->Address1) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E11003');
				$this->Pstatus = 0;
			}				
			if(trim($this->City) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E11004');
				$this->Pstatus = 0;
			}				
			if(trim($this->Zip) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E11005');
				$this->Pstatus = 0;
			}					
			if(trim($this->Country) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E11006');
				$this->Pstatus = 0;
			} 				
			if(trim($this->State) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E11007');
				$this->Pstatus = 0;
			}					
			if(trim($this->PhoneNumber) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E11008');
				$this->Pstatus = 0;
			}		
		}
		/* End */
		...
		public function CodeValidate() {
			if(trim($this->UniqueCode) == '' && $this->Pstatus == 1){
				$this->SetStatus(0, 'E4017');
				$this->Pstatus = 0;
			}
			if($this->CheckCodeExistence($this->UniqueCode) == false && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4002');
				$this->Pstatus = 0;
			}
			if($this->IsAlreadyRegistered($this->UniqueCode) && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E4001');
				$this->Pstatus = 0;
			}
		}
		...
		private function ValidateForgetPasswordInput() {
			if(trim($this->EmailID) == '' && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E7005');
				$this->Pstatus = 0;
			}
			if($this->CheckEmailExistance($this->EmailID) == false) {
		...
		public function ResetDB() {
			...
			if(trim($this->Password) != trim($this->ConfirmPassword) && $this->Pstatus == 1) {
				$this->SetStatus(0, 'E7008');
				$this->status = 0;
			}
			if($this->Pstatus == 1) {
				$DataArray = array(
					"RU_Password"	=> PassEnc($this->Password),
					"RU_UpdatedDate"=> getDateTime());

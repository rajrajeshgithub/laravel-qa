<?php
	class Fundraisers_Model extends Model
	{
		...
		public function GetFundraiserDetails($DataArray=array('Camp_ID','Camp_RUID','Camp_PaymentMode','Camp_TeamUserType','Camp_Code','Camp_CP_FirstName','Camp_CP_LastName','Camp_Status','Camp_Level_ID','Camp_StylingTemplateName'),$where='')
		{
		...
		{
				$Fields=implode(",",$DataArray);
				$Sql="SELECT $Fields FROM ".TBLPREFIX."campaign C LEFT JOIN ".TBLPREFIX."registeredusers RU ON(C.Camp_RUID=RU.RU_ID) WHERE 1=1 ";
		...
		public function ProcessFundraiserBasicDetail()
		{
			...
			if($this->P_status==1)
			{
				if($this->F_Camp_Code!='')
				{
					$DataArray = array("Camp_Title"=>$this->F_Camp_Title,"Camp_DescriptionHTML"=>$this->F_Camp_DescriptionHTML,"Camp_SalesForceID"=>$this->F_Camp_SalesForceID,"Camp_SocialMediaUrl"=>$this->F_Camp_SocialMediaUrl,"Camp_UrlFriendlyName"=>$this->F_Camp_UrlFriendlyName,"Camp_IsPrivate"=>$this->F_Camp_IsPrivate,"Camp_LastUpdatedDate"=>getDateTime(),"Camp_Locale"=>GetUserLocale(),"Camp_StylingDetails"=>$this->Camp_StylingDetails);
		...
		public function GetFundraiserComment()
		{
			...
			$where='';
			//LIMIT
			$limit=' ORDER by Camp_Cmt_ID DESC LIMIT '.($this->FC_PageNo-1)*$this->FC_PageLimit.' , '.$this->FC_PageLimit;
			$sql="SELECT Camp_Cmt_ID, Camp_Cmt_RUID, Camp_Cmt_UserName, Camp_Cmt_CampID, Camp_Cmt_Comment, DATE_FORMAT(Camp_Cmt_CreatedDate,'%b, %D %Y') as Camp_Cmt_CreatedDate, Camp_Cmt_LastUpdatedDate, Camp_Cmt_ShowOnWebsite FROM ".TBLPREFIX."campaigncomments WHERE Camp_Cmt_CampID='".$this->FC_FundraiserId."'";

<?php
	class Ut1_Reporting_Model extends Model {
		...
		public function GetDonationDetails($DataArray) {
			...
			//new code
			if(isset($this->LoginUserDetails['UserType1']['user_id']))
		...
		public function GetDonationFundDetails($DataArray) {
			$Fields	= implode(",", $DataArray);
			$Sql = "SELECT $Fields FROM " . TBLPREFIX . "purchasedonationdetails PDD INNER JOIN " . TBLPREFIX . "registeredusers RU ON(RU.RU_ID = PDD.PDD_RUID) 
					LEFT JOIN " .TBLPREFIX."campaign C ON(C.Camp_ID=PDD.PDD_CampID)
					LEFT JOIN " . TBLPREFIX . "paymenttransection PT ON (PDD.PDD_PaymentTransactionID=PT.PT_ID)";

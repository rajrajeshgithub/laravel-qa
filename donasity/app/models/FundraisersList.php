<?php
	class FundraisersList_Model extends Model
	{
		...
		public $KeywordFilter,$CategoryFilter,$LocationFilter,$TeamcodeFilter,$SelectedCategoryArray=array();
		public $PageSelected;
		...
		public function GetNPOList($fieldArray, $filters, $keyword='', $activePage='')
		{
			...
			$OrderBy=' ORDER by Camp_Title ASC, Camp_ShortDescription ASC';
			...
			$sqlCount	= "SELECT  count(C.Camp_ID) FROM ".TBLPREFIX."campaign C INNER JOIN ".TBLPREFIX."npocategories CC on(C.Camp_Cat_ID=CC.NPOCat_ID)";
		...
		private function ManageNPOListArray()
		{
			...
			{
				$CategoryName = isset($val['NPO_CatName']) ? explode('||', $val['NPO_CatName']) : array(0=>'', 1=>'');
				$val['NPO_State'] = isset($val['NPO_State']) ? $this->GetStateName($val['NPO_State']) : '';
		...
		{	
			...
			if(isset($Filter['teamcode']) && $Filter['teamcode'] != NULL)
				$this->TeamcodeFilter	= $Filter['teamcode'];
		...
		{
			$this->npoCondition	= " WHERE 1=1 AND Camp_Status=15 AND Camp_IsPrivate!='1' AND Camp_Deleted!='1' AND Camp_DemoUse!='1' AND Camp_EndDate>='".getDateTime(0,'Y-m-d')."'";
			$this->ManageKeywordFilter();
			$this->ManageCategoryFilter();
			$this->ManageTeamcodeFilter();
		...
		private function ManageKeywordFilter()
		{
			...
			{
				$this->npoFilterArray['keyword'] = " ((C.Camp_Title LIKE '%".$this->KeywordFilter."%' or C.Camp_ShortDescription LIKE '%".$this->KeywordFilter."%' or C.Camp_DescriptionHTML LIKE '%".$this->KeywordFilter."%'))";		
			}
		}
		...
		private function ManageTeamcodeFilter()
		{
			if(isset($this->TeamcodeFilter) && $this->TeamcodeFilter != "")
			{
				$this->npoFilterArray['teamcode'] = " ((C.Camp_Code = '".$this->TeamcodeFilter."'))";	
			}
		}

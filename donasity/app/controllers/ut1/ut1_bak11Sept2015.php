<?php
	class Ut1_Controller extends Controller
	{
		...
		private function ForgetPassword()
		{
			...
			if($this->objutype1->Pstatus)
			{
				$Link	= '<a href="'.$this->objutype1->Pageurl.'">Click to reset your password</a>';	
				echo $Link;exit;
				$this->tplVarArray	= array('tpl_name'=>$this->objutype1->UserDetailsArray['Name'],'tpl_link'=>$Link,'tpl_url'=>"",
											'tpl_mail'=>$this->objutype1->EmailAddress);
				$this->ntfVarArray	= array('ntf_EmailTo'=>$this->objutype1->EmailAddress);
				//$this->SendEmail();
				redirect(URL."ut1");
			}
			else

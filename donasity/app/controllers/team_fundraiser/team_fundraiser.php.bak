<?php
	...
	class Team_Fundraiser_Controller extends Controller
	{
		...
		public function showCaptainFundraiserPage()
		{
			...
			$DataArray = array("CONCAT(RU_FistName,' ',RU_LastName) as FullName", "C.Camp_ID","Camp_Code","Camp_TeamUserType","Camp_PaymentMode","Camp_StylingTemplateName","Camp_Title",
								"Camp_ShortDescription","camp_thumbImage","camp_bgImage","Camp_UrlFriendlyName","ROUND(Camp_DonationGoal) as Camp_DonationGoal","Camp_Location_City","Camp_Location_State",
								"Camp_Location_Country","Camp_Location_Logitude","Camp_Location_Latitude",
								"ROUND(Camp_DonationReceived) as Camp_DonationReceived","concat(round(( Camp_DonationReceived/Camp_DonationGoal * 100 ),0),'%') AS Donationpercentage",
								"Camp_StartDate", "Camp_EndDate");
		...
		public function joinFundraiser()
		{
			...
			$DataArray	= array('RU.RU_ID','RU.RU_FistName','RU.RU_LastName','RU.RU_EmailID','RU.RU_ProfileImage','RU.RU_City','RU.RU_ZipCode','RU.RU_State','RU.RU_Country','RU.RU_Address1','RU.RU_Address2','RU.RU_EmailID');
				...
				if($this->C_ID)
				{
					...
					$sMessage = "Process to join team fundraiser successfull";
					$lMessage = "Process to join team fundraiser successfull";
		...
		private function createDataArray()
		{
			...
			$DataArray = array("Camp_Cat_ID" => $this->arrCampaign['Camp_Cat_ID'],
									...
									"Camp_ShortDescription" => $this->arrCampaign['Camp_ShortDescription'],
									...
									"Camp_StylingDetails" => $this->rrCampaign['Camp_StylingDetails'],
		...
		private function sendMailCaptain($FR_id)
		{
			...
			$Keyword='JoinedTeamFundraiserCaptain';

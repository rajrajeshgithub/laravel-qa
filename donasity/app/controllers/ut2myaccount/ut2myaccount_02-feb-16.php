...
*/
	...
	{		
		...
		public $sortfrom, $sortto, $P_status = 0;
		...
		public function exportdonationlist()
		{
			...
			$this->objut2report->SortOrder=" PDD.PDD_DateTime DESC ";					
			$this->objut2report->NPOEIN=$NPODetails["NPOEIN"];	
			$DonationArray	= $this->objut2report->GetDonationDetails(array('PDD.PDD_PIItemName','PDD.PDD_TaxExempt','DATE_FORMAT(PDD.PDD_DateTime,"%m/%d/%Y")as PDD_DateTime','PDD.PDD_Cost','CONCAT(RU.RU_FistName," ",RU.RU_LastName)DonorName'));
			//dump($DonationArray);
		...
		private function getFundariserList()
		{
			...
			$Wherecondition = " AND Camp_RUID=".keyDecrypt($this->LoginUserDetail['UserType2']['user_id'])." AND Camp_Deleted!='1' AND Camp_TeamUserType NOT IN('T','C')";
			...
			$Wherecondition = " AND Camp_RUID=".keyDecrypt($this->LoginUserDetail['UserType2']['user_id'])." AND Camp_Deleted!='1' AND Camp_TeamUserType IN('T','C')";
		...
		public function FundraiserEdit($CampID='') {
			...
			$this->tpl->assign($this->objCom->GetPageCMSDetails('ut2_fundraiser_edit'));
		...
		public function FundraiserComment($FundraiserID)
		{
			...
			if(trim($FundraiserID)!='') 
			{
				...
				$fundraiserComment=$this->objFund->GetFundraiserComment();
					...
					$this->tpl->assign($this->objCom->GetPageCMSDetails('ut2_fundraiser_comment'));
					$this->tpl->assign('arrBottomInfo',$this->objCom->GetCMSPageList(BOTTOM_URL_NAVIGATION));
					$this->tpl->assign('MenuArray',$this->objCom->getTopNavigationArray(LANG_ID));
					$this->tpl->assign('arrBottomMetaInfo',$this->objCom->GetPageCMSDetails(BOTTOM_META));
		...
		{
			$this->load_model('Common','objCom');
			$arrMetaInfo	= $this->objCom->GetPageCMSDetails('ut2_fundraiser_basic_detail');
		...
		{
			$this->load_model('Common','objCom');
			$arrMetaInfo = $this->objCom->GetPageCMSDetails('TeamFundraiserBasicDetailUT2');
			//dump($arrMetaInfo);
			$DataArray = array('*','concat_ws(", ",Camp_Location_City,Camp_Location_State,Camp_Location_Country) as Camp_Location');
			$this->objFund->F_Camp_ID = keyDecrypt($FR_id);
...
'SendMode'=>'1','AddedOn'=>getDateTime());
		...
		public function FundraiserPhotoVideo($FR_id)
		{
			...
			$arrMetaInfo	= $this->objCom->GetPageCMSDetails('ut2_edit_photo_video');
		...
		public function viewallfundraiser($CampID='') {
			...
			if($CampID == '')
				redirect(URL . "ut2myaccount/fundarisers-list");
		...
		public function manageTeamFundraisers($CampID)
		{
			...
			$DataArray=array('Camp_ID','Camp_RUID','Camp_Code','Camp_TeamUserType','Camp_Status','Camp_Level_ID','Camp_StylingTemplateName');

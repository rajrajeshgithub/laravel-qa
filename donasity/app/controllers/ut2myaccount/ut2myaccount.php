...
if(!$this->objutype2->checkLogin(getSession('Users')))redirect(URL."ut2/npo-login");
		...
		public function exportdonationlist()
		{
			...
			$this->objut2report->SortOrder=" PDD.PDD_DateTime DESC ";					
			$this->objut2report->NPOEIN=$NPODetails["NPOEIN"];	
			$DonationArray	= $this->objut2report->GetDonationDetails(array('PDD.PDD_PIItemName','PDD.PDD_TaxExempt','DATE_FORMAT(PDD.PDD_DateTime,"%m/%d/%Y")as PDD_DateTime','PDD.PDD_Cost','CONCAT(RU.RU_FistName," ",RU.RU_LastName)DonorName','RU.RU_FistName','RU.RU_LastName','REPLACE(RU.RU_EmailID,"_DNB","")RU_EmailID','RU.RU_Phone','CONCAT(RU.RU_Address1," ",RU_Address2)DonorAddress','RU.RU_City','RU.RU_State','RU.RU_ZipCode'));
			//dump($DonationArray);
		...
		private function CreateCsvFile()
		{
			...
			if($fp)
			{
			$HeaderArr	= array("Donation To","Tax Exempt","Date of Donation ","Donation Amount","Donor Full Name","Donor First Name","Donor Last Name","Email","Phone","Address","City","State Prov","Zip","Country");
		...
		private function Dashboard()
		{
			...
			$Limit =" limit 0,20 ";
			$where='';		
			$DataArrayx = array('PDD.PDD_RUID','PDD.PDD_PIItemName','PDD.PDD_PD_ID','PDD.PDD_DateTime','PDD.PDD_SubTotal','PDD.PDD_Cost','PDD.PDD_TaxExempt','PDD_PIItemType','PDD.PDD_DonationReciptentType','CONCAT(RU.RU_FistName," ",RU.RU_LastName)DonorName','REPLACE(RU.RU_EmailID,"_DNB","")RU_EmailID');
			$DonationArray	= $this->objut2report->GetDonationDetails($DataArrayx,$where,$Limit);			
...
'SendMode'=>'1','AddedOn'=>getDateTime());
		...
		public function exportfundraiserlist() {
			...
			$fund_array = array('PDD.PDD_PIItemName','PDD.PDD_TaxExempt','DATE_FORMAT(PDD.PDD_DateTime,"%m/%d/%Y")as PDD_DateTime','PDD.PDD_Cost','CONCAT(RU.RU_FistName," ",RU.RU_LastName)DonorName','RU.RU_FistName','RU.RU_LastName','REPLACE(RU.RU_EmailID,"_DNB","")RU_EmailID','RU.RU_Phone','CONCAT(RU.RU_Address1," ",RU_Address2)DonorAddress','RU.RU_City','RU.RU_State','RU.RU_ZipCode');
		...
		{			
			...
			if(count($listTeamFundraisers)>0)
			{
				...
				{
					//$arrNameEmail_ids = explode("-",trim($strEmailId));	cannot explode on '-' breaks emails such as oliver-g.brown@donasity.com
					$arrNameEmail_ids[0] = trim($strEmailId);
				...
				if($cntSuccess==count($arrEmail_ids))
					...
					$this->SetStatus(0,'000',"Email not sent to below addresses<br/>".$invalidEmails);
			}
			redirect(URL.'ut2myaccount/manageTeamFundraisers/'.keyEncrypt($cap_fund_id));	
		}
		...
		private function sentInvitationEmail($receiverEmail,$mailContent)
		{
			...
			///$arrNameEmail_ids[1] = explode("-",trim($receiverEmail));	 exploding on '-' breaks emails such as oliver-g.brown@donasity.com
			$arrNameEmail_ids[1] = $receiverEmail;	
			...
			$InsertDataArray=array('FromID'=>$UserId,
			'CC'=>$GetTemplate['EmailToCc'],'BCC'=>$GetTemplate['EmailToBcc'],
			'FromAddress'=>$GetTemplate['EmailFrom'],'ToAddress'=>$arrNameEmail_ids[1],
			'Subject'=>$GetTemplate['Subject_'._DBLANG_],'Body'=>$HTML,'Status'=>'0','SendMode'=>'1','AddedOn'=>getDateTime());
			//dump($InsertDataArray);
			$id=$this->objemail->InsertEmailDetail($InsertDataArray);
			$Eobj	= LoadLib('BulkEmail');
		...
		public function deleteTeamMember($camp_id,$c_camp_id)
		{
			...
			//$captn_camp_id = keyDecrypt($c_camp_id);
			...
			/*----update process log------*/
			$sMessage = "Error in deleting team fundraiser.";
			$lMessage = "Error in deleting team fundraiser.";
			if($this->P_status) {
		...
		public function deltUnfinishedFundraiser($FR_id,$Status,$redirect=0)
		{
			$FR_id = keyDecrypt($FR_id);
			$Status = keyDecrypt($Status);// status really has no bearing here, just using it as a nice key to confirm no hacking			
			if($Status==15)
			{
				$pStatus = 0;
				$deleteString=1;
				$sMessage = "Error attempting to delete a fundraiser";
				$lMessage = "Error attempting to delete a fundraiser";
				$userType = $this->LoginUserDetail['UserType2']['user_type'];
				
				$DataArray = array("Camp_Deleted"=>$deleteString);
				if($this->objFund->SetFundraiserDetails($DataArray,$FR_id))
					$pStatus = 1;
					
				if($pStatus) {
					$sMessage = "Fundraiser deleted successfully.";
					$lMessage = "Fundraiser deleted successfully.";
				}
				$DataArray = array(	
					"UType"			=> $userType == '1' ? 'UT1' : $userType == '2' ? 'UT2' : '',
					"UID"			=> keyDecrypt($this->LoginUserDetail['UserType2']['user_id']),
					"UName"			=> $this->LoginUserDetail['UserType2']['user_fullname'],
					"RecordId"		=> $FR_id,
					"SMessage"		=> $sMessage,
					"LMessage"		=> $lMessage,
					"Date"			=> getDateTime(),
				...
				if($redirect==1)
					redirect(URL.'ut2myaccount/manageTeamFundraisers/'.keyEncrypt($FR_id));
				elseif($redirect==2)
					redirect(URL.'ut2myaccount/fundraisers-list'); // only using this one at present, will keep others for future possible use
				else				
					redirect(URL.'ut2myaccount/fundraisers-list/'); // wouldn't go back to edit page of a deleted fundraiser, so changed this one
			}

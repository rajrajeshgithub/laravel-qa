<?php
	class fundraiserdetail_Controller extends Controller
	{
			...
			try
			{
				 if($this->F_Status == 1) {
					...
					if($UType1 != 0) {
				  	...
				  	$this->objFund->F_WhereCondition = " AND Camp_Status<=15 AND (Camp_RUID=" . $UType1 . " OR Camp_RUID=" . $UType2 . ")";
				 }
		...
		private function ShowTemplateV2() {
			...
			if($this->objFund->F_FundraiserDetail['Details']['Camp_NPO_EIN'] != '')
			...
			$tpl->assign('FundraiseID', $this->objFund->F_Camp_ID);
			$tpl->assign('F_FundraiserDetail', $fundraiserDetails);
			$tpl->assign('TotalDonor', $TotalDonor[0]['TotalDonor'] == '' ? 0 : $TotalDonor[0]['TotalDonor']);

...
*/
	...
	{		
		...
		{		
			...
			$this->objutype2->logoutUT2();	wasn't this supposed to log out a u2 if a u2 logged in as a u1? test */
		...
		public function index($type='dashboard')
		{
			...
			switch($type)
			{
				...
				case 'fundarisers-list':
					...
					break;	// to help those pages not yet fixed as below
		...
		private function getFundraiserList()
		{
			...
			$Wherecondition = " AND Camp_RUID=".keyDecrypt($this->LoginUserDetail['UserType1']['user_id'])." AND Camp_Deleted!='1'";
			...
			$fundraiserlist_arrayTeam = $fundraiserlist_array;
		...
		private function Dashboard()
		{
			...
			$this->objut1report->LoginUserID = 		keyDecrypt($this->LoginUserDetail['UserType1']['user_id']);	
		...
		public function getFundraiserCommentHTML($res)
		{
			...
			for($i=0;$i<count($res);$i++)
			{
				...
				$html.='<h3 class="comment-heading colorTheme-section-head">'.$res[$i]['Camp_Cmt_UserName'].'</h3>';
		...
		public function UpdateFundraiserBasicDetail()
		{
			...
			$FR_id=request('post','FR_id',0);
			$this->objFund->Image = $_FILES['uploadPhoto'];
			$this->objFund->camp_bgImage = isset($_FILES['camp_bgImage']) ? $_FILES['camp_bgImage'] : array();
		...
		private function SendFundraiserMailAfterUpdateOwner($FR_id)
		{
				...
				$InsertDataArray=array('FromID'=>isset($FundraiserDetail['Camp_RUID']) ? $FundraiserDetail['Camp_RUID'] : 0,
				'CC'=>$GetTemplate['EmailToCc'],'BCC'=>$GetTemplate['EmailToBcc'],
...
'SendMode'=>'1','AddedOn'=>getDateTime());
		...
		public function updateStatus($FR_id,$Status,$redirect=0)
		{
			...
			if($Status==15||$Status==21 || $Status==36)
			{
				...
				if($pStatus) {
					$sMessage = "Fundraiser has stopped successfully.";
					$lMessage = "Fundraiser has stopped successfully.";
				}
				$DataArray = array(	
					"UType"			=> $userType == '1' ? 'UT1' : $userType == '2' ? 'UT2' : '',
					"UID"			=> keyDecrypt($this->LoginUserDetail['UserType1']['user_id']),
				...
				elseif($redirect==2)
					redirect(URL.'ut1myaccount/fundraisers-list');
				else				
		...
		{			
			...
			$mailContent = request('post','messageContent',0);
			if($mailContent=='' || $mailContent=='0'){$mailContent='none';}
			//dump($arrCamp_ids,0);
			if($csEmail_ids=='')
			redirect(URL.'ut1myaccount/manageTeamFundraisers/'.keyEncrypt($cap_fund_id));
			...
			if(count($listTeamFundraisers)>0)
			{
				...
				{
					//$arrNameEmail_ids = explode("-",trim($strEmailId));  	not sure why exploding this way, breaks emails such as oliver-g.brown@donasity.com	
					$arrNameEmail_ids[0] = $strEmailId;	
				...
				if($cntSuccess==count($arrEmail_ids))
					...
					$this->SetStatus(0,'000',"An email was not able to be sent for the below addresses<br/>".$invalidEmails);
			}
			redirect(URL.'ut1myaccount/manageTeamFundraisers/'.keyEncrypt($cap_fund_id));	
		}
		...
		private function sentInvitationEmail($receiverEmail,$mailContent)
		{
			...
			$GetTemplate=$this->objemail->GetTemplateDetail($DataArray,$where);
			$LoginUserDetail = getSession('Users','UserType1');
			//$arrNameEmail_ids = explode("-", trim($receiverEmail));       cannot explode on '-' breaks oliver-g.brown@donasity.com
		...
		public function deltUnfinishedFundraiser($FR_id,$Status,$redirect=0)
		{
			...
			if($Status==15)
			{
				...
				if($redirect==1)
					redirect(URL.'ut1myaccount/manageTeamFundraisers/'.keyEncrypt($FR_id));
				elseif($redirect==2)
					redirect(URL.'ut1myaccount/fundraisers-list'); // only using this one at present, will keep others for future possible use
				else				
					redirect(URL.'ut1myaccount/fundraisers-list/'); // wouldn't go back to edit page of a deleted fundraiser, so changed this one
			}

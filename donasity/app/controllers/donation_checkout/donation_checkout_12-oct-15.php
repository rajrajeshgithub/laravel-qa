<?php
	class Donation_checkout_Controller extends Controller
	{
		...
		function __construct()
		{
			...
			//$this->Ref_ID	= getSession("DonasityLoginDetail","cart_ref_id");
			$this->tpl = new view();
		}
		
		public function index($type='donation_checkout',$ID=NULL,$Donor='NPOR')
		{
		...
		{
			$FundRaiserArray=array();$NPORArray=array();$NPONRArray=array();
			if($this->Donor == 'FUNDARISER')
			{
				...
				//dump($Detail);
				$FundRaiserArray	= array('ID'=>$this->ID,'Title'=>$Detail[0]['Camp_Title'],'Recurring'=>0,'Recurring_Mode'=>'','Recurring_Year'=>'','Donation_Amount'=>'','Transaction_Fee'=>'','Payable'=>0,
											'Product_Type'=>'CD','itemcode'=>'CD1','City'=>$Detail[0]['Camp_Location_City'],'Category'=>$Detail[0]['Camp_Cat_ID'],
									'Zipcode'=>"",'State'=>$Detail[0]['Camp_Location_State'],'Connected_UserID'=>$Detail[0]['Camp_RUID'],'Connected_Stripe_AccountID'=>$Detail[0]['Camp_Stripe_ConnectedID'],"TaxExempt"=>($Detail[0]['Camp_TaxExempt']==1)?1:0);
				//dump($FundRaiserArray);
			}
			elseif($this->Donor == 'NPOR')
			{	
				$Detail		= $this->GetNpoDetails($this->ID);	
				$NPORArray	= array('ID'=>$this->ID,'Title'=>$Detail['NPO_Name'],'Recurring'=>0,'Recurring_Mode'=>'','Recurring_Year'=>'','Donation_Amount'=>'','Transaction_Fee'=>'','Payable'=>0,'Product_Type'=>'NPOD','itemcode'=>'NPOD1',
									'City'=>$Detail['NPO_City'],'Category'=>$Detail['category'],
									'Zipcode'=>$Detail['NPO_Zip'],'State'=>$Detail['NPO_State'],'Connected_UserID'=>$Detail['USERID'],'Connected_Stripe_AccountID'=>$Detail['Stripe_ClientID'],"TaxExempt"=>($Detail['NPO_DedCode']==1)?1:0);	
			}
			elseif($this->Donor == 'NPONR')
			{
				$Detail		= $this->GetNpoDetails($this->ID);	
				$NPONRArray	= array('ID'=>$this->ID,'Title'=>$Detail['NPO_Name'],'Recurring'=>0,'Recurring_Mode'=>'','Recurring_Year'=>'','Donation_Amount'=>'','Payable'=>0,'Transaction_Fee'=>'','Product_Type'=>'NPOD','itemcode'=>'NPOD2',
									'City'=>$Detail['NPO_City'],'Category'=>$Detail['category'],
									'Zipcode'=>$Detail['NPO_Zip'],'State'=>$Detail['NPO_State'],'Connected_UserID'=>$Detail['USERID'],'Connected_Stripe_AccountID'=>$Detail['Stripe_ClientID'],"TaxExempt"=>($Detail['NPO_DedCode']==1)?1:0);	
			}
...
*/	
			...
			if($Cart == "")
			{
				...
				$this->CartArray['calculation']	= array('Total'=>0,'TransactionFeePaidByUser'=>1,'TransactionRate'=>TRANSACTION_FEE,'TransactionFee'=>0,'TotalDonation'=>0,'TotalPay'=>0);
		...
		private function Cart()
		{
			...
			if(trim($continue_to_donate_url)=="") $continue_to_donate_url="/campaign/index/campaigncategorylist";
			

			$this->CartArray	= getCookie('DonatiobasketArray');//dump($this->CartArray);

			foreach(array("NPOR","NPONR","FUNDARISER") as $key)
			if(isset($this->CartArray[$key])) $cartCount=$cartCount+count($this->CartArray[$key]);

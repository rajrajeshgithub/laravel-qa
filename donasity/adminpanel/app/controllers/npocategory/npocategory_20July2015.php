<?php
	class Npocategory_Controller extends Controller
	{
		...
		{
			$this->RelationCatParam	= " WHERE 1=1 AND NPO_SubSectionName <> '' ";
			$Condition	= request('post','condition',1);
			$Keyword	= request('post','keyword',0);
			
			if($Condition == 0 && $Keyword!='')
			{
				$this->RelationCatParam.=" AND (NPO_SubSectionName LIKE '%".$Keyword."%')";	
			}
			...
			{
				$this->RelationCatParam.=" AND (NPO_SubSectionName = '".$Keyword."')";	
			}
		...
		private function NPORelationCategory()
		{
			...
			$Order		= " ORDER BY NPO_SubSectionName";	
			$SubSection	= $this->objNpoCategory->GetNPOSubSectionName($DataArray,$this->RelationCatParam,$Order);
			$SortedSubSection	= array();
			foreach($SubSection as $key => &$val)
			{
				$val['CatID']	= $this->GetExistRelationID($val['NPO_SubSectionName']);	
				if($val['CatID'] == 0)
				{

<?php
	class Npocategory_Controller extends Controller
	{
		...
		private function CreateCategoryRelation()
		{
			...
			$category		= request('post','category',3);
			//echo "<pre>";print_r($CategoryID);print_r($ExistCatIDArr);dump($SubSectionName);
			//dump($category);
			$Status			= true;
			/*echo "<pre>";
			print_r($CategoryID);
			...
			foreach($CategoryID as $key => $CatID)
			{
				if($CatID > 0){
					...
					$Category	= $this->objNpoCategory->GetNpoCategoryName(array('NPOCat_ID','NPOCat_DisplayName_EN','NPOCat_DisplayName_ES'),array('NPOCat_ID'=>$CatID));
					$catName	= $Category['NPOCat_DisplayName_EN']." || ".$Category['NPOCat_DisplayName_ES'];
					$Status	= $this->objNpoCategory->CreateCategoryRelationDB($Subsection,$catName,$CatID);
				}
			}	
			if($Status)
			{

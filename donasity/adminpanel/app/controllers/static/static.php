...
class Static_Controller extends Controller
{
	...
	{
		$Array	= array('CMSPagesID','CMSPageGroupID','internal_url','IsInternalLink','external_url','IsExternalLink','CMSPagesName','CMSPagesNameINURL','CMSPagesTitleEN','CMSPagesTitleES','CMSPageHead','Content','ContentES',
	...
	{
		$groupdetail = array(array('Title'=>'', 'SortingOrder'=>'', 'Status'=>''));
	...
	public function validatePageInput()
	{
		...
		if(trim($this->DataArray['CMSPagesTitleEN'])== '' )
		{
			$this->setErrorMsg('E3005');
			redirect($_SERVER['HTTP_REFERER']);
		}
		if(trim($this->DataArray['CMSPagesTitleES'])== '' )
		{
			$this->setErrorMsg('E3009');
			redirect($_SERVER['HTTP_REFERER']);
		}
	}
	...
	private function PageInputData()
	{
		...
		$PageTitleEN		= request('post','pagetitle_EN',0);
		$PageTitleES		= request('post','pagetitle_ES',0);
		...
		$CmsPageHead        = request('post','CmsPageHead',4);
		$landingPageHtml	= request('post','landingpageHtml',4);
		
		$SP_landingpageHtml	= request('post','SP_landingpageHtml',4);	
		...
		$this->DataArray			= array('CMSPageGroupID'=>$GroupID,
									...
									'CMSPagesTitleEN'=>$PageTitleEN,
									'CMSPagesTitleES'=>$PageTitleES,									
									'CMSPageHead'=>addslashes($CmsPageHead),
									'Content'=>addslashes($landingPageHtml),
									'ContentES'=>addslashes($SP_landingpageHtml),
									'Metatitle'=>$MetaTitle,

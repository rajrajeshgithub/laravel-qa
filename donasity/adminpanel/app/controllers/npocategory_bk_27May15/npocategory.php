<?php
	class Npocategory_Controller extends Controller
	{
		...
		private function getFormData()
		{
			...
			$this->FieldArr			= array("NPOCat_ParentID"=>$this->ParentCategory,"NPOCat_DisplayName_EN"=>$this->CategoryNameEn,"NPOCat_DisplayName_ES"=>$this->CategoryNameES,"NPOCat_CodeName"=>$this->CategoryCode,"NPOCat_URLFriendlyName"=>$this->UrlFriendlyName,
		...
		private function CheckDuplicacyForCategoryCode($condition)
		{
			...
			if(trim($this->FieldArr['NPOCat_CodeName'])<>'')
			{
				$searchField = " WHERE (NPOCat_CodeName='".$this->FieldArr['NPOCat_CodeName']."')";
				$CategoryCode = $this->objNpoCategory->CheckDuplicacyForCategoryCode($condition,$searchField);
				
				if(count($CategoryCode)>0)
				{
					$KeywordStatus=FALSE;
					$this->setErrorMsg('E10007');	
				}
			}else{
				$KeywordStatus=FALSE;
				$this->setErrorMsg('E10008');
			}

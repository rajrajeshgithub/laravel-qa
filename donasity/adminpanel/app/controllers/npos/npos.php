...
class Npos_Controller extends Controller
{
	...
	{
		$DataArray	= array("ND.NPO_ID", "ND.NPO_EIN", "ND.NPO_Name", "ND.NPO_ICO", "ND.NPO_Street", "ND.NPO_City", "ND.NPO_State", "ND.NPO_Zip", "ND.NPO_SubSectionName", "ND.NPO_SubSectionDesc", "ND.NPO_CD", "ND.NPO_Category", "ND.NPO_IRS_NTEECode2Description", "ND.NPO_Affiliation", "ND.NPO_AffType", "ND.NPO_IRS_AffiliationCodesDescription", "ND.NPO_DedCode", "ND.NPO_DedDescription", "ND.NPO_FoundationCode", "ND.NPO_FoundationDescription", "ND.NPO_OrgCode", "ND.NPO_OrgDescription", "ND.NPO_EO_StatusCode", "ND.NPO_EO_Description", "ND.NPO_AssetIncomeCode", "ND.NPO_FilingCode",  "ND.NPO_IRS_AssetIncomeCodesDescription", "ND.NPO_IRS_FilingCodesDescription", "NPO_PF_FilingCode","ND.NPO_IRS_PF_FilingCodesDescription", "ND.NPO_AssetAmt", "ND.NPO_IncomeAmt", "ND.NPO_RevenueAmt","ND.NPO_Status", "ND.NPO_CreatedDate", "ND.NPO_LastUpdatedDate", "ND.NPO_LastUpdatedBy","ND.NPO_DisplayOnWebsite", "ND.NPO_UniqueCode", "IF(ISNULL(NUR.ID),0,1) AS IsRegistered");
		$Condition	= " WHERE ND.NPO_ID=" . $this->nposID;
	...
	private function NPOsList()
	{
		...
		if($_SERVER['QUERY_STRING'] == NULL){
			...
			$DataArray = array('N.NPO_ID','N.NPO_EIN','N.NPO_Name','N.NPO_City','N.NPO_DedCode','N.NPO_Status','N.NPO_State as State_Name','CR.NPO_CatName', 'IF(ISNULL(NUR.ID),0,1) AS IsRegistered');
		...
		$this->tpl->assign('totalnpos', $Page_totalRecords);
	...
	private function FilterData()
	{
		...
		if($SearchCategory != '' && $SearchCategory != NULL)
			$ConditionArr[]	= "CR.NPOCat_ID  = '" . $SearchCategory . "'";
		
		if($SearchState != '' && $SearchState != NULL)
			$ConditionArr[]	= "N.NPO_State = '" . $SearchState . "'";
			
		if($SearchCity != '' && $SearchCity != NULL)
			$ConditionArr[]	= "N.NPO_City LIKE '%".$SearchCity."%'";
			
		if($Status !='' && $Status != NULL)
			$ConditionArr[]	= "N.NPO_Status = '" . $Status . "'";
			
		if($DeductionMode != '' && $DeductionMode != NULL)
			$ConditionArr[]	= "N.NPO_DedCode = '" . $DeductionMode . "'";
		
		if($CompareCondition == 0) {
			if(trim($Keyword) != "" && $Fields != "")
				$ConditionArr[]	= $Fields . " LIKE '%" . $Keyword . "%'";
			elseif(trim($Keyword) != "" && $Fields == "") {
				$ConditionArr[]	= "(N.NPO_Name LIKE '%" . $Keyword . "%' OR N.NPO_EIN LIKE '%" . $Keyword . "%' OR N.NPO_ICO LIKE '%" . $Keyword . "%' OR N.NPO_Street LIKE '%" . $Keyword . "%' OR N.NPO_City LIKE '%" . $Keyword . "%' OR N.NPO_State LIKE '%" . $Keyword . "%' OR N.NPO_Zip LIKE '%" . $Keyword  ."%')";	
			}
		}
		...
		elseif($CompareCondition == 2)
		{
			...
			elseif(trim($Keyword) != "" && $Fields == "") {
				$ConditionArr[]	= "(N.NPO_Name='" . $Keyword . "' OR N.NPO_EIN='" . $Keyword . "' OR N.NPO_ICO='" . $Keyword . "' OR N.NPO_Street='" . $Keyword . "' OR N.NPO_City='" . $Keyword . "' OR N.NPO_State='" . $Keyword . "' OR N.NPO_Zip='" . $Keyword . "')";
			}	
		}
		
		$ConditionStr = implode(' AND ', $ConditionArr);
		if($ConditionStr != "")
			$Condition = " WHERE " . $ConditionStr;

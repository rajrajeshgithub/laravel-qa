...
class Npos_Controller extends Controller
{
	...
	private function GetNPOsDetail()
	{
		$DataArray	= array("NPO_ID","NPO_EIN","NPO_Name","NPO_ICO","NPO_Street","NPO_City","NPO_State","NPO_Zip","NPO_SubSectionName","NPO_SubSectionDesc","NPO_CD","NPO_Category",
							...
							"NPO_RevenueAmt","NPO_Status","NPO_CreatedDate","NPO_LastUpdatedDate","NPO_LastUpdatedBy","NPO_DisplayOnWebsite");
	...
	{
		$Condition 	= $this->FilterData();	
		$pageSelected = (int)request('post', 'pageNumber', 1);
		$this->objnpos->pageSelectedPage	= ($pageSelected==0)?1:$pageSelected;
		$DataArray	= array('N.NPO_ID','N.NPO_EIN','N.NPO_Name','N.NPO_ICO','N.NPO_Street','N.NPO_City','N.NPO_State','N.NPO_Zip','N.NPO_Status','S.State_Name');
	...
	private function FilterData()
	{
		$Condition			= "";
		$ConditionArr		= array();
		$SearchCategory		= request('post','category',0);
		$SearchState		= request('post','state',0);
		$SearchCity				= request('post','city',0);
		$Status				= "";
		if($_POST['status'] != ""){$Status= request('post','status',1);}
		$DeductionMode		= request('post','deductionmode',0);
		$Fields				= request('post','fields',0);
		$CompareCondition	= request('post','condition',1);
		$Keyword			= request('post','keyword',0);
		$CategoryParentID	= request('post','categoryparentid',1);
		
		if($CategoryParentID > 0){
		$this->getSubCategoryList($CategoryParentID);
		}
		
		if($SearchCategory !='' && $SearchCategory!= NULL){$ConditionArr[]	= "N.NPO_SubSectionName  = '".$SearchCategory."'";}
		if($SearchState !='' && $SearchState!= NULL){$ConditionArr[]	= "N.NPO_State = '".$SearchState."'";}
		if($SearchCity !='' && $SearchCity!= NULL){$ConditionArr[]	= "N.NPO_City = '".$SearchCity."'";}
	...
	private function GetDeductionMode()
	{
		$DeductionMode	= $this->objnpos->GetDeductionModeDB();	
		$this->tpl->assign("DeductionMode",$DeductionMode);
	}
	...
	Private function GetBankDetails($EIN)
	{
		$DataArray	= array("NPO_BD_EIN","NPO_BD_BankName","NPO_BD_BankAddress","NPO_BD_Phone","NPO_BD_EmailAddress","NPO_BD_AccountType","NPO_BD_AccountName","NPO_BD_AccountNumber",
							"NPO_BD_PreferredPaymentMode","NPO_BD_ID");

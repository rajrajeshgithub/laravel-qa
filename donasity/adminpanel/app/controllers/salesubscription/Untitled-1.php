<?php
	class Salesubscription_Controller extends Controller
	{
		...
		public function process($SS_Id=NULL)
		{
			if($SS_Id!=NULL)
		...
		public function addOrder($RU_Id=NULL)
		{
			if($RU_Id!=NULL)
			{	
		...
		public function showEditOrderForm($SS_Id=NULL)
		{
			if($SS_Id!=NULL)
				$this->SS_Id = keyDecrypt($SS_Id);
			else
			...
			$this->objSaleSub->SS_Id = $this->SS_Id;
			$arraySaleSubDetails = $this->objSaleSub->getSaleSubscriptionDetails($FieldArray);		
			if($arraySaleSubDetails['SS_Status']<=4)
				redirect(URL.'salesubscription/process/'.$SS_Id);
			$this->tpl = new view;
		...
		private function showList()
		{
			...
			$statusArray = array(1=>'Pending',2=>'Order Added',3=>'Waiting For Payment',4=>'Waiting For Payment Details From Customer', 11=>'Ready To Charge',15=>'Activated',21=>'Stoped by Administrator');
			...
			$this->tpl->assign('statusArray',$statusArray);			
			$this->tpl->assign('arraySaleSubDetails',$arraySaleSubDetails);
		...
		private function showStep1()
		{
			...
			$this->tpl->assign('isEdit',$this->isEdit);			
		...
		private function showStep2()
		{
			...
			$this->objSaleSub->SS_Id = $this->SS_Id;
			$arraySaleSubDetails = $this->objSaleSub->getSaleSubscriptionDetails($FieldArray);		
			/*if($arraySaleSubDetails['SS_Status']>=3 && $arraySaleSubDetails['SS_Status']<=4) 
			{
				redirect(URL.'salesubscription/process/'.keyEncrypt($this->SS_Id));
			}
			elseif($arraySaleSubDetails['SS_Status']>10 && $arraySaleSubDetails['SS_PaySimplePaymentMethodID']!='')
			{
				redirect(URL.'salesubscription/process');
			}		*/	
			$this->tpl = new view;
		...
		public function updatePaymentMethod()
		{
			...
			{			
				...
				if($this->objUSAePay->updateCustomerPaymentMethod())
				{
					...
					$DataArray = array('SS_Status'=>3,'SS_LastUpdatedDate'=>getDateTime(),'SS_Locale'=>GetUserLocale());
					$this->objSaleSub->SS_Id = $this->SS_Id;
					$this->objSaleSub->updateSaleSubscriptionDetails($DataArray);				
			...
			$sMessage 	= "Error in update payment method ";
			$lMessage 	= "Error in add payment method with Method id -".$inputDataArray['methodId'];
			if($this->objUSAePay->P_Status)
			{	
				$sMessage = "Payment method updated";
				$lMessage = "Payment method updated with Method id -".$inputDataArray['methodId'];
			}
			...
			$DataArray = array(	"UType"=>$userType,
								...
								"Model"=>get_class($this->objSaleSub));	
			$this->objSaleSub->updateProcessLog($DataArray);*/	
			/*-----------------------------*/
			redirect(URL."salesubscription/process/".keyEncrypt($this->SS_Id));	
		}
		...
		public function processStep1()
		{
			...
			{	
				...
				redirect(URL.'salesubscription/addOrder');
			}
			...
			if($inputDataArray['SS_Id']!='')
			{
				...
				$ProcessStatus = $this->arraySaleSubDetails['SS_Status'];
				if($ProcessStatus==1) /*If process status - Order Added - it update form details else add details*/
				{
					...
					$DataArray = array( 'SS_DateTime'=>getDateTime(),
										...
										'SS_Schedule'=>isset($inputDataArray['interval'])?$inputDataArray['interval']:NULL,										
					...
					$userID		= $this->loginDetails['admin_id'];
					$userName	= $this->loginDetails['admin_fullname'];
					$sMessage 	= "Error in udpate sale subscription details step 1";
					$lMessage 	= "Error in udpate sale subscription details step 1";
					if($this->objSaleSub->P_Status)
					{
						$sMessage = "Step 1 sale subscription details updated";
						$lMessage = "Step 1 sale subscription details updated";
					}
						$DataArray = array(	"UType"=>$userType,
											"UID"=>$userID,
											"UName"=>$userName,
				...
				{
					redirect(URL.'salesubscription/process/'.keyEncrypt($inputDataArray['SS_Id']));
				}
			}
			else
			{
				...
				if($this->SS_Id)
				{
					...
					$userID		= $this->loginDetails['admin_id'];
					$userName	= $this->loginDetails['admin_fullname'];
					$sMessage 	= "Error in add sale subscription details step 1";
					$lMessage 	= "Error in add sale subscription details step 1";
					if($this->objSaleSub->P_Status)
					{
						$sMessage = "Step 1 sale subscription details added";
						$lMessage = "Step 1 sale subscription details added";
					}
						$DataArray = array(	"UType"=>$userType,
											"UID"=>$userID,
											"UName"=>$userName,
			...
			if($this->SS_Id)
			{
				...
				if(count($arraySaleSubDetails)>0)
				{					
					$this->load_model('Usaepay', 'objUSAePay');
					$this->objUSAePay->ArrayCustomerDetails = array('BillingAddress' => array(
																		'FirstName'=>$arraySaleSubDetails['SS_FirstName'],
					...
					{	
						...
						$userID		= $this->loginDetails['admin_id'];
						$userName	= $this->loginDetails['admin_fullname'];
						$sMessage 	= "Error in udpate sale subscription details";
						$lMessage 	= "Error in udpate sale subscription details";
						if($this->objSaleSub->P_Status)
						{
							$sMessage = "Sale subscription details updated";
							$lMessage = "Sale subscription details updated";
						}
							$DataArray = array(	"UType"=>$userType,
												"UID"=>$userID,
												"UName"=>$userName,
		...
		{			
			...
			if($ProcessStatus==1)
				redirect(URL.'salesubscription/process/'.keyEncrypt($this->SS_Id));
			elseif($ProcessStatus>10 && $ProcessStatus<=15 && $arraySaleSubDetails['SS_PaySimplePaymentMethodID']!='')
			...
			if($this->objUSAePay->addCustomerPaymentMethod())
			{
				...
				//{
					//$this->SetStatus(1,'C21002');
				//}
				$redirection = URL."salesubscription/showEditOrderForm/".keyEncrypt($this->SS_Id);	
			}
			else
			{
				$this->SetStatus(0,000,$this->objUSAePay->ErrorMessage);		
				$redirection = URL.'salesubscription/process/'.keyEncrypt($this->SS_Id);								
			}
			...
			$userID		= $this->loginDetails['admin_id'];
			$userName	= $this->loginDetails['admin_fullname'];
			$sMessage 	= "Error in add payment method with PaySimple customer id - ".$arraySaleSubDetails['SS_PaySimpleCustomerID'];
			$lMessage 	= "Error in add payment method with PaySimple customer id - ".$arraySaleSubDetails['SS_PaySimpleCustomerID'];
			if($this->objUSAePay->P_Status)
			{	
				$sMessage = "Payment method added with PaySimple customer id - ".$arraySaleSubDetails['SS_PaySimpleCustomerID'];
				$lMessage = "Payment method added with PaySimple customer id - ".$arraySaleSubDetails['SS_PaySimpleCustomerID'];
			}
		...
		public function processSendLinkCustomerPaymentDetails($ss_Id)
		{
			...
			$FieldArray = array('SS_ID','SS_FirstName','SS_LastName','SS_EmailAddress','SS_PaySimpleCustomerID','SS_Status','SS_Schedule','SS_ItemName','SS_Amount','SS_RefNumber','SS_CreatedDate');
			$this->objSaleSub->SS_Id = $this->SS_Id;
			$arraySaleSubDetails = $this->objSaleSub->getSaleSubscriptionDetails($FieldArray);
			if(count($arraySaleSubDetails)>0)
			{
				if($this->sendLinkMailCustomerPaymentDetails($arraySaleSubDetails))
				{
					...
					$DataArray = array('SS_Status'=>4,'SS_LastUpdatedDate'=>getDateTime(),'UpdateLog'=>"CONCAT(UpdateLog,'".$logText."')");
					$this->objSaleSub->SS_Id = $this->SS_Id;
					$this->objSaleSub->updateSaleSubscriptionDetails($DataArray);
		...
		private function sendLinkMailCustomerPaymentDetails($saleSubDetails)
		{
			...
			$link = "<a href='".FRONTURL."salesubscription/".keyEncryptFront($saleSubDetails['SS_ID'])."/' style='color:#abc340;' style='color:#abc340;'/>Add Payment Details</a>";
			$link_url = FRONTURL."salesubscription/".keyEncryptFront($saleSubDetails['SS_ID']);
		...
		public function markActive($ss_id)
		{
			$this->SS_Id = keyDecrypt($ss_id);			
			...
			{
				if($ArraySaleSubscription['SS_Status']==11)
				{
					$this->load_model('Usaepay', 'objUSAePay');
					$arrayTransStatus = get_setting('TransactionStatus');
						...
						if($this->objUSAePay->runCustomerTransaction())
						{
							...
							if(strtolower($this->objUSAePay->PaySimpleResponseArray['Result'])=='approved')
								...
								if($intStatus==1)
									...
									$logText = "Updated Details for Activation After Create Transaction on USAePay for Status :".$this->objUSAePay->PaySimpleResponseArray['Status']."  - Updated by : System , Updated on : ".formatDate(getDateTime(),'n-j-y h:ia')." <br/>"; /*log*/
									...
									if($this->objSaleSub->P_Status)
									{
										if($ArraySaleSubscription['SS_EnableRecipt'])
										{
											...
											$ArraySaleSubscription['PaidAmount'] = $this->objUSAePay->PaySimpleResponseArray['AuthAmount'];										
			...
			if(count($arraySaleSubDetails)>0)
			{
				if($arraySaleSubDetails['SS_Status']==11)
				{
					$logText = "Updated details after mark Activate - Updated by (Admin): ".$this->loginDetails['admin_fullname']." Updated on : ".formatDate(getDateTime(),'n-j-y h:ia')." <br/>"; 
			...
			redirect(URL."salesubscription/showEditOrderForm/".keyEncrypt($this->SS_Id));			
		}

...
class NposProcess_Controller extends Controller
{
	...
	public $CurrentCsvPosition=0,$TotalRowProcessed=1,$EndCsv=0,$filePath,$fileSize,$dataHeadarr,$dataCsvArr=array(),$CsvLimit=3,$ExpCsvLimit=50,$Log_Key,$filename,$HeaderArr=array();
	...
	public function ValidateInputCsv()
	{
		...
		if($this->P_status == 1)
		{
			...
			else if(!validateExtension($this->CSVFile,array("csv","zip")))
			{
				$this->setErrorMsg('E6001');
				redirect(URL."npos/index/import-csvfile");
			}
					...
					if($this->fileSize>0)
					{
						...
						while(!feof($handle))
						{
							...
							else
							{
								...
								//remove extra valuee
								$ExtraIndex	= getSession('arrCsv','EXTRAINDEX');
	...
	private function InitErrorRange()
	{
		if(($this->TotalRowProcessed == 0))
		{
			$this->StartErrorRange	= 	$this->TotalRowProcessed;
			$this->EndRange	= ($this->StartErrorRange+($this->CsvLimit));	
		}
		else
		{
			$this->StartErrorRange	=	getSession('arrCsv','ENDRANGE');
			$this->EndRange	= ($this->StartErrorRange+$this->CsvLimit);
		}
		
		$filename	= getSession('arrCsv','CURCSVFILENAME');
		$this->TotalRows	= 	count(file(IMPORT_CSV_PATH.$filename))-1;
		$this->EndRange	=	($this->EndRange > $this->TotalRows)?$this->TotalRows:$this->EndRange;
		setSession('arrCsv',$this->EndRange,'ENDRANGE');
	}
	...
	public function ViewRedirectCsv()
	{
		...
		$this->NPOsDataInsert();
		//if($this->P_status == 0)$this->NPOsCategoryInsert();
		$tpl = new view;
		$string = file_get_contents(NPOSIMPERRJSONFILE.$this->IPStr.".json");
		$this->json_array = json_decode($string, true);
		$tpl->assign('errorarr',$this->json_array);
		if($this->P_status == 0)
		 unlink(NPOSIMPERRJSONFILE.$this->IPStr.".json");
	...
	{
		$State			= substr(trim($Data[6]),0,2);		
		$EIN			= substr(trim($Data[1]),0,2);
		$NPOName		= substr(trim($Data[2]),0,1);//echo "State===".$State." and EIN === ".$EIN." and NPOName ==".$NPOName;exit;
	...
	public function NPOsDataInsert()
	{
		...
		{			
			...
			{
				$UniqueCode	= $this->generateuniquecode($val);
				$arrValue = array();
				$Value .= "(";
				//echo "<pre>";
				//print_r($this->dataCsvArr);
			...
			$this->objnposprocess->InsertNPOsData($Query);// or die($this->errorFn(mysql_error()));
		}
		catch(Exception $e)
		{
			$error	=	$e->getMessage()."===Line No. ==".$e->getLine()."===File===".$e->getFile();
			$this->errorlog($error);			
			
			$ErrorArray	= array("Error found between ".($this->StartErrorRange+1)." to ".$this->EndRange);
			
			if($this->P_status == 1)
			{
				$this->json_array	= array();
				if(file_exists(NPOSIMPERRJSONFILE.$this->IPStr.".json")){
					$string = file_get_contents(NPOSIMPERRJSONFILE.$this->IPStr.".json");
					$this->json_array = json_decode($string, true);
					if(count($this->json_array)>0){
						$MainErrorArray	= array_merge($this->json_array,$ErrorArray);
					}
					else
					{
						$MainErrorArray		= $ErrorArray;	
					}
				}
				else
				{
				...
				$ErrorArray	= json_encode($MainErrorArray);
				$fp = fopen(NPOSIMPERRJSONFILE.$this->IPStr.".json", 'w');
				fwrite($fp, $ErrorArray);
				fclose($fp);
			}
		}
	...
	private function errorlog($error)
	{
		...
		{
			$error_str='\n\n\n\n\n\n\n\n\n'.$error;
			file_put_contents(NPOSIMPERRLOGFILE, $error_str, FILE_APPEND);
		}
	}
	...
	private function ValidateHeader()
	{
		$Array		= array(//"ID"=>"NPO_ID",
							...
							"ACCESS_CODE"=>"NPO_UniqueCode");			
			
			//dump($this->HeaderArr);				
			//remove extra header and dava value
			$this->ExtraIndex	= array();//echo "<pre>";print_r($this->HeaderArr);
			foreach($this->HeaderArr as $Index => $value)
			{
			...
			setSession('arrCsv',$this->ExtraIndex,'EXTRAINDEX');
			//dump($this->ExtraIndex);
			/*===============*/
		...
		$this->FieldsArr	= array_filter($this->FieldsArr);
		//dump($this->FieldsArr);
	...
	private function NPOsCategoryInsert()
	{
		...
		if(count($Category) > 0)
		{
			...
			{
				if($Cat['NPO_SubSectionName'] != ""){
					$Cat['NPO_SubSectionName'] = str_replace(",","\,",$Cat['NPO_SubSectionName']);
				$Array[]	= "('".addslashes($Cat['NPO_SubSectionName'])."','".addslashes($Cat['NPO_SubSectionName'])."','".addslashes($Cat['NPO_SubSectionName'])."',
								'".addslashes($Cat['NPO_SubSectionName'])."')";	
				}
			}
	...
	{
		$Category	= $this->objnposprocess->GetDistinctCategoryDB();
		return $Category;
	}
	...
	private function CreateCsvFile()
	{
		...
		{			
			$HeaderArr	= array("ID","EIN","NAME","ICO","STREET","CITY","STATE","ZIP","Subsection Name","Subsection Description","NTEE_CD","Category","IRS_NTEE_Code2_Description",
								...
								"PF_Filing_Code","IRS_PF_Filing_Codes_Description","ASSET_AMT","INCOME_AMT","REVENUE_AMT","ACCESS_CODE");
	...
	{
		$LimitStr	= " LIMIT ".$this->CurrentCsvPosition.",".$this->ExpCsvLimit;
		$DataArray	= array("N.NPO_ID","N.NPO_EIN","N.NPO_Name","N.NPO_ICO","N.NPO_Street","N.NPO_City","N.NPO_State","N.NPO_Zip","N.NPO_SubSectionName","N.NPO_SubSectionDesc","N.NPO_CD",
							"N.NPO_Category",
							"N.NPO_IRS_NTEECode2Description","N.NPO_Affiliation","N.NPO_AffType","N.NPO_IRS_AffiliationCodesDescription","N.NPO_DedCode","N.NPO_DedDescription","N.NPO_FoundationCode",
							"N.NPO_FoundationDescription","N.NPO_OrgCode","N.NPO_OrgDescription","N.NPO_EO_StatusCode","N.NPO_EO_Description","N.NPO_AssetIncomeCode",
							"N.NPO_IRS_AssetIncomeCodesDescription","N.NPO_FilingCode","N.NPO_IRS_FilingCodesDescription","N.NPO_PF_FilingCode","N.NPO_IRS_PF_FilingCodesDescription","N.NPO_AssetAmt",
							"N.NPO_IncomeAmt","N.NPO_RevenueAmt","N.NPO_UniqueCode");

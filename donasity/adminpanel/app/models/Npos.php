...
class Npos_Model extends Model {
	...
	public function GetNPOsDetailDB($DataArray, $Condition) {
		...
		$Sql	= "SELECT $Fields FROM " . TBLPREFIX . "npodetails ND LEFT JOIN " . TBLPREFIX . "npouserrelation NUR ON (ND.NPO_ID = NUR.NPOID)";
		//dump($Sql . $Condition);
	...
	public function GetNPOsList_DB($DataArray, $Cond) {
		...
		$Sql = "SELECT $fields FROM " . TBLPREFIX . "npodetails N LEFT JOIN " . TBLPREFIX . "npocategoryrelation CR ON (CR.NPO_CategoryName=N.NPO_CD) LEFT JOIN " . TBLPREFIX . "npouserrelation NUR ON (N.NPO_EIN=NUR.NPOEIN)";
		$Order = " GROUP BY N.NPO_ID ORDER BY N.NPO_ID";
		...
		/*$sqlCount = "SELECT count(NPO_ID) FROM " . TBLPREFIX . "npodetails N LEFT JOIN " . TBLPREFIX . "npocategoryrelation CR ON (CR.NPO_CategoryName=N.NPO_CD) LEFT JOIN " . TBLPREFIX . "npouserrelation NUR ON (N.NPO_EIN=NUR.NPOEIN)";*/

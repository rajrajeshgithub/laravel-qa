...
class Npos_Model extends Model
{
	...
	public function GetNPOsDetailDB($DataArray,$Condition)
	{
		...
		$Res	= db::get_row($Sql.$Condition);//dump($Res);
	...
	public function GetNPOsList_DB($DataArray,$Cond)
	{
		...
		$Sql	= "SELECT $fields FROM ".TBLPREFIX."npodetails N 
				   LEFT JOIN ".TBLPREFIX."states S ON (N.NPO_State=S.State_Value)
		...
		$GroupBy	= " GROUP BY N.NPO_EIN ";
		$StartIndex	= ($this->pageSelectedPage-1)*$this->pageLimit;
		$Limit = " LIMIT ".$StartIndex.", ".$this->pageLimit;
		$Res	= db::get_all($Sql.$Cond.$GroupBy.$Order.$Limit);//echo $Sql.$Cond.$GroupBy.$Order.$Limit;
		ini_set("memory_limit",-1);
		$this->TotalCountNPOs	= db::count($Sql.$Cond.$GroupBy);
	...
	public function GetContactDetailsDB($DataArray,$EIN)
	{
		...
		$Res	= db::get_all($Sql);//echo $Sql;exit;
		return (count($Res)>0)?$Res:array();
	}

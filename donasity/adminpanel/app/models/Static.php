...
class Static_Model extends Model {
	...
	public $ErrorMessage, $ErrorCode, $ConfirmCode, $ConfirmMsg, $MsgType = 2, $ErrorCodes = '';
	...
	public function GetGroup($Array, $Cond='', $Order='') {
		$Where	= " WHERE 1 AND Language='en'" . $Cond;
	...
	public function GetPages($Array, $Cond='', $Order='') {
		$fields	= implode(',', $Array);
		$Where	= " WHERE 1 AND CP.Language='en'";
		$Where	= $Where . $Cond;
		$Sql	= "SELECT $fields FROM " . TBLPREFIX . "cmspages as CP INNER JOIN " . TBLPREFIX . "cmspagegroup AS CPG ON CP.CMSPageGroupID = CPG.CMSPageGroupID";
		$Res	= db::get_all($Sql . $Where . $Order);	
		return (count($Res) > 0) ? $Res : array();
	}
	...
	public function CheckDuplicacyForCMSGroup($condition) {
		...
		$row	= db::get_row($sql . $Where);
		return ($row['CMSPageGroupID'] > 0) ? false : true;
		exit;
	}
	...
	public function CheckPageNameDuplicacy($condition) {
		...
		$row	= db::get_row($sql.$Where);
		return ($row['CMSPagesID'] > 0) ? false : true;
		exit;	
	}
